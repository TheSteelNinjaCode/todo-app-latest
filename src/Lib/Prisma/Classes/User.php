...
class User implements IModel
{
    ...
    public function __construct($pdo, $data = null)
    {
        ...
        $this->_fieldsRelated = ['userRole', 'Todo'];
        ...
        if ($data) {
            ...
            $this->Todo = new Todo($this->_pdo, $data['Todo'] ?? null);
        }
    }
    ...
    protected function connectUserRole(string $relationName, array|bool $connectData, string $lastInsertId, string $connectType = 'connect')
    {
        ...
        $relationModel = new UserRole($this->_pdo);
    ...
    protected function includeTodo(array $items, array $selectedFields = [], array $includeSelectedFields = [], bool $format = false) 
    {
        if (empty($items)) {
            return $items;
        }
        ...
        $instanceModelName = new Todo($this->_pdo);
        foreach ($items as &$item) {
    ...
    protected function connectTodo(string $relationName, array $connectData, string $lastInsertId, string $connectType = 'connect')
    {
        $dbType = $this->_dbType;
        $quotedTableName = $dbType == 'pgsql' ? "\"Users\"" : "`Users`";
        $tableName = $dbType == 'pgsql' ? "\"Users\"" : "`Users`";
        ...
        $relationModel = new Todo($this->_pdo);
        ...
        if (!empty($foundUnique) && $connectType === 'connect') {
            try {
                ...
                throw new \Exception("Error connecting Todo: " . $e->getMessage());
            }
        }
        ...
        if (!empty($foundUnique) && $connectType === 'disconnect') {
            try {
                ...
                throw new \Exception("Error disconnecting Todo: " . $e->getMessage());
            }
        }
    }
                ...
                foreach ($data as $relationName => $relationDataName) {
                    ...
                    if (in_array($relationName, $relationNames)) {
                        ...
                        if (isset($relationDataName['create']) && is_array($relationDataName['create'])) {
                            ...
                            if ($checkArrayContentType !== ArrayType::Value) {
                                throw new \Exception("To create a new record, the value of 'create' must be a single array with names as keys and the data to create as values in Todo model. example: ['create' => ['name' => 'someName']]");
                            }
                            ...
                            if ($checkArrayContentType !== ArrayType::Associative) {
                                throw new \Exception("To create many records, use an associative array with the field names as keys and the data to create as values in Todo model. ['createMany' => [['name' => 'someName'], ['name' => 'someOtherName']]");
                            }
                    ...
                    if (isset($relationDataName['connect']) || isset($relationDataName['connectOrCreate']) || isset($relationDataName['disconnect']) || isset($relationDataName['update']) || isset($relationDataName['updateMany'])) {
                        ...
                        if (isset($relationDataName['connect'])) {
                            ...
                            if (isset($relationDataName['connect']) && $checkArrayContentType !== ArrayType::Value) {
                                throw new \Exception("The 'connect' key must be an associative array with the field names as keys and the data to create as values related Todo model. example: ['connect' => ['id' => 'someId']]");
                            }
                        }
                        ...
                        if (isset($relationDataName['connectOrCreate'])) {
                            ...
                            if (isset($relationDataName['connectOrCreate']) && $checkArrayContentType !== ArrayType::Associative) {
                                throw new \Exception("The 'connectOrCreate' key must be an associative array with the field names as keys and the data to create as values related Todo model. example: ['connectOrCreate' => ['where' => ['id' => 'someId'], 'create' => ['name' => 'someName']]");
                            }
                        }
                        ...
                        if (isset($relationDataName['disconnect'])) {
                            ...
                            if (!is_bool($disconnectData)) {
                                ...
                                if (isset($relationDataName['disconnect']) && $checkArrayContentType !== ArrayType::Value) {
                                    throw new \Exception("The 'disconnect' key must be an associative array with the field names as keys and the data to create as values related Todo model. example: ['disconnect' => ['id' => 'someId']]");
                                }
                            }
                        }
    ...
    public function aggregate(array $operation, bool $format = false): array | object
    {
        ...
        $quotedTableName = $this->_dbType == 'pgsql' ? "\"Users\"" : "`Users`";
        $conditions = [];
        $bindings = [];
