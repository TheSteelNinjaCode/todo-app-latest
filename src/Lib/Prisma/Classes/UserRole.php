...
class UserRole implements IModel
{
    ...
    protected function includeUser(array $items, array $selectedFields = [], array $includeSelectedFields = [], bool $format = false) 
    {
        ...
        $tableName = $dbType == 'pgsql' ? "\"userRole\"" : "`userRole`";
        $modelName = 'User';
        $relationName = 'user';
    ...
    protected function connectUser(string $relationName, array $connectData, string $lastInsertId, string $connectType = 'connect')
    {
        $dbType = $this->_dbType;
        $quotedTableName = $dbType == 'pgsql' ? "\"UserRole\"" : "`UserRole`";
        $tableName = $dbType == 'pgsql' ? "\"userRole\"" : "`userRole`";
    ...
    {
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required when creating a new UserRole.");
        }
        ...
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (!isset($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required when creating a new UserRole.");
            }
        }
                ...
                foreach ($data as $relationName => $relationDataName) {
                    ...
                    if (in_array($relationName, $relationNames)) {
                        ...
                        if (isset($relationDataName['create']) || isset($relationDataName['createMany']) || isset($relationDataName['connect']) || isset($relationDataName['connectOrCreate'])) {
                            throw new \Exception("The relation name '$relationName' is not defined in the UserRole model.");
                        }
                    }
                }
            }
        ...
        foreach ($includes as $relation => $relationInclude) {
            if ($relationInclude) {
                ...
                if (method_exists($this, $includeMethodName)) {
                    ...
                    throw new \Exception("The '$relation' does not exist, in the UserRole model.");
                }
            }
        }
    ...
    public function update(array $data, bool $format = false): array | object
    {
        ...
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (isset($data['data'][$fieldName]) && empty($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required in UserRole Model.");
            }
        }
    ...
    public function aggregate(array $operation, bool $format = false): array | object
    {
        ...
        $quotedTableName = $this->_dbType == 'pgsql' ? "\"UserRole\"" : "`UserRole`";
        $conditions = [];
        $bindings = [];
    ...
    /**
     * Groups records in the 'UserRole' table and performs aggregate operations.
     * 
    ...
    {
        if (!isset($data['where'])) {
            throw new \Exception("The 'where' key is required in the updateMany UserRole.");
        }
        ...
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required in the updateMany UserRole.");
        }
